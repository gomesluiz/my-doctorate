...
for (project.name in projects){
  ...
  # save data to dnn 
  set.seed(DEFAULT_SEED)
  reports$class <- as.factor(ifelse(reports$bug_fix_time <= 365, 0, 1))
  ...
  train.data    <- reports.dataset[in_train, ]
  test.data     <- reports.dataset[-in_train, ]
  write.csv(train.data, sprintf("20190917_%s_bug_report_train_data.csv", project.name), row.names=F)
  write.csv(test.data,  sprintf("20190917_%s_bug_report_test_data.csv",  project.name), row.names=F)
  # 
