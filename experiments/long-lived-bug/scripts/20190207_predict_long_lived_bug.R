...
resampling <- c("none", "boot", "cv", "loocv", "lgocv", "repeatedcv", "timeslice")
classifier <- c("knn", "nb", "nnet", "rf", "svmRadial")
n_term     <- c(100, 200, 300, 400, 500)
threshold  <- seq(4, fixed.threshold, by = 4)
parameters <- crossing(feature, classifier, resampling, threshold, n_term)
...
# rf
  ...
  predictors <- names(all_reports)[!(names(all_reports) %in% c("bug_id", "days_to_resolve", class_label))]
