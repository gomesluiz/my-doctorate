...
for (i in oss.projects) {
  
  reports.file <- file.path(DATADIR, sprintf("20190309_%s_bug_report_data.csv", i))
  ...
  #parameter = parameters[i, ]
    ...
    flog.trace("Text mining: extracting %d terms", n_term[1])
    source <- reports[, c('bug_id', feature[1])]
    corpus <- clean_corpus(source)

    dtm  <- tidy(make_dtm(corpus, n_term[1]))
  ...
  for (j in oss.projects) {
    
    if (i != j) next 
    
    set.seed(1234)
    flog.trace("Cross Predicting: [%s] versus [%s]", i, j)
    
  #flog.trace("Balancing dataset")
  #dataset = balance_dataset(all_reports, class_label, parameter$balancing)
    predictors  <- !(names(all_reports) %in% c("bug_id", "days_to_resolve", class_label))
    #predictors <- !(names(all_reports) %in% c(class_label))

  #flog.trace("Partitioning dataset")
  #in_train <- createDataPartition(dataset[, class_label], p = 0.75, list = FALSE)

  #X_train <- dataset[, predictors]
  #y_train <- dataset[, class_label]

    X_test  <- all_reports[, predictors]
    y_test  <- all_reports[, class_label]

    model.file <- file.path(DATADIR, sprintf("20190315_%s_final_model.rds", j))
    model.fit  <- readRDS(model.file)
  ...
  #
    ...
    y_hat <- predict(object = model.fit, X_test)
  ...
  metrics.mask  <- sprintf("cross_dataset_%s_%s_result_metrics_.csv", i, j)
  metrics.file  <- format_file_name(DATADIR, timestamp, metrics.mask)
