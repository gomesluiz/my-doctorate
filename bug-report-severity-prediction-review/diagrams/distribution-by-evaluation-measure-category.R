...
## Preparing data
sheet.name   <- "distribution-by-evaluation-measure"
...
categories = c("Single Class" , "Multi-Class" , "Summary" ,"Graphical")
...
papers.all=list(  Single.Class=unique(papers.c1$value)
                , Multi.Class=unique(papers.c2$value)
                , Summary=unique(papers.c3$value)
                , Graphical=unique(papers.c4$value)
)
tikz(file.name.tex, width = width.cm, height = height.cm, standAlone = TRUE)
upset(fromList(papers.all), 
       sets = c("Single.Class" , "Multi.Class" , "Summary" ,"Graphical")
