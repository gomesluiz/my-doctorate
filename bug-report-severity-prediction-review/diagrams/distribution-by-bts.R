...
fill.colors <- c('gray90', 'gray90', 'gray90')
...
categories = c("Bugzilla" , "Google" , "Jira")
...
papers.all=list(  Bugzilla=unique(papers.c1$value)
                , Google=unique(papers.c2$value)
                , Jira=unique(papers.c3$value))
tikz(file.name.tex, width = width.cm, height = height.cm, standAlone = TRUE)
