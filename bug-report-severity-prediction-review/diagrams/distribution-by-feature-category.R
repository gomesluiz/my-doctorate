...
## Plot upset chart
file.name.tex <- paste(sheet.name, '-in-ups.tex', sep='')
file.name.pdf <- paste(sheet.name, '-in-ups.pdf', sep='')
...
papers.all=list(  Unstructured.Text=unique(papers.c1$value)
                , Qualitative.Categorical=unique(papers.c2$value)
                , Qualitative.Ordinal=unique(papers.c3$value)
                , Quantitative.Discrete=unique(papers.c4$value)
                , Quantitative.Continuous=unique(papers.c5$value)
                )
tikz(file.name.tex, width = width.cm, height = height.cm, standAlone = TRUE)
upset(fromList(papers.all), 
      sets = c(  "Unstructured.Text"
