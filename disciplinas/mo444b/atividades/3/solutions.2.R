...
NormalizeData <- function(data){
  # Standardizes the columns for media 0 and standard deviation 1
  # 
  # Args:
...
CalculateAccuracyForNnet <- function(data, classes){
  ...
  for(e in external.folds) {
    ...
    if ( internal.accuracy >  internal.max.accuracy ){
      ...
      internal.best.decay   <- nnet.fit$bestTine$decay
      cat(sprintf("[Nnet] size= %d, accuracy = %.5f\n", 
  ...
  external.nnet.fit <- train(Class ~ ., 
                             ...
                             method="nnet",
                             trControl=nnet.control,
                             tuneGrid=expand.grid(size=c(internal.best.size), decay=c(internal.best.decay)), 
                             MaxNWts=5000, 
                             verbose=FALSE)
...
main <- function() {
  ...
  # output <- rbind(output, data.frame(classifier="svm",accuracy=nnet.accuracy))
