...
/**
 * The <code>FileResource</code> class represents a file and allows access to
 * its contents a line at a time, using the method <code>lines</code>, or a word
 * at a time, using the method <code>words</code>. These strings can then be
 ...
 *
 * <P>
 * If each line of the file represents separated data values, because its a CSV
 * file, then the user can get a <code>getCSVParser</code> object to access that
 * data more directly, using one of the <code>getCSVParser</code> methods.
 *
 * <P>
 * Example CSV usage:
 * 
...
public class FileResource {
	...
	/**
	 * Create a <code>FileResource</code> object that opens the file chosen by
	 * the user using a file selection dialog box.
	 * 
	 * @throws exception
	 ...
	 * by the File object passed as a parameter.
	 * 
	 * Useful, for example, when used in conjunction with the
	 * <code>DirectoryResource</code> class.
	 * 
	 * @param file
	 *            the file to be represented by this resource
	 * @throws exception
	...
	/**
	 * Create a <code>FileResource</code> object that opens a file whose name is
	 ...
	 * @throws exception
	 *             if the filename cannot be accessed
	 */
	...
	/**
	 * Create a <code>FileResource</code> object that opens a file represented
	 * by the File object passed as a parameter, possibly to write to it.
	 * 
	 * If the user wants to change the contents of the open file by using the
	 * method <code>write</code> to add new strings to it, pass
	 * <code>true</code> as the second parameter. Otherwise it is assumed the
	 * user will only iterate over the existing contents of the file.
	 * 
	 * Useful, for example, when used in conjunction with the
	 * <code>DirectoryResource</code> class.
	 * 
	 ...
	 *            the file to be represented by this resource
	 * @param writable
	 *            allow changes to this file only if true
	 * @throws exception
	 *             if the file cannot be accessed
	 */
	 ...
	 * 
	 * @return an <code>Iterable</code> that will allow access to contents of
	 *         opened file one line at a time.
	 */
	...
	/**
	 * Allow access to this opened file one word at a time, where words are
	 * separated by white-space. This means any form of spaces, like tabs or
	 * newlines, can delimit words.
	 * 
	 * @return an <code>Iterable</code> that will allow access to contents of
	 *         opened file one word at a time.
	 */
	...
	/**
	 * Returns a <code>CSVParser</code> object to access the contents of an open
	 * file.
	 * 
	 * Each line of the file should be formatted as data separated by commas and
	 * with a header row to describe the column names.
	 * 
	 * @return a <code>CSVParser</code> that can provide access to the records
	 *         in the file one at a time
	 * @throws exception
	 *             if this file does not represent a CSV formatted data
	 */
	...
	/**
	 * Returns a <code>CSVParser</code> object to access the contents of an open
	 * file, possibly without a header row.
	 * 
	 * Each line of the file should be formatted as data separated by commas and
	 * with/without a header row to describe the column names.
	 * 
	 * @param withHeader
	 *            uses first row of data as a header row only if true
	 * @return a <code>CSVParser</code> that can provide access to the records
	 *         in the file one at a time
	 * @throws exception
	 *             if this file does not represent a CSV formatted data
	...
	/**
	 * Returns a <code>CSVParser</code> object to access the contents of an open
	 * file, possibly without a header row and a different data delimiter than a
	 * comma.
	 * 
	 * Each line of the file should be formatted as data separated by the
	 * delimiter passed as a parameter and with/without a header row to describe
	 * the column names. This is useful if the data is separated by some
	 * character other than a comma.
	 * 
	 * @param withHeader
	 *            uses first row of data as a header row only if true
	 * @param delimiter
	 *            a single character that separates one field of data from
	 *            another
	 * @return a <code>CSVParser</code> that can provide access to the records
	 *         in the file one at a time
	 * @throws exception
	 *             if this file does not represent a CSV formatted data
	 * @throws exception
	 *             if <code>delimiter.length() != 1</code>
	 */
	...
	/**
	 * Allows access to the column names of the header row of a CSV file (the
	 * first line in the file) one at a time. If the CSV file did not have a
	 * header row, then an empty <code>Iterator</code> is returned.
	 * 
	 * @param parser
	 *            the <code>CSVParser</code> that has been created for this file
	 * @return an <code>Iterable</code> that allows access one header name at a
	 *         time
	 */
	 ...
	 * 
	 * @param s
	 *            the string to saved to the file
	 */
